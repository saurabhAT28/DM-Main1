...
def app(data):
    ...
    operation = st.selectbox("Operation", [
                             "Chi-Square Test", 'Correlation(Pearson) Coefficient', 'Normalization Techniques'])
    cols = []
    ...
    if operation == "Chi-Square Test":
        ...
        chiSquare = 0.0
        for i in arrClass:
            chiSquare += (v.loc[attribute1][i]-(((v[i].sum())*(v1[attribute1].sum()))/total))*(v.loc[attribute1][i]-(
                ((v[i].sum())*(v1[attribute1].sum()))/total))/(((v[i].sum())*(v1[attribute1].sum()))/total)
            chiSquare += (v.loc[attribute2][i]-(((v[i].sum())*(v1[attribute2].sum()))/total))*(v.loc[attribute2][i]-(
                ((v[i].sum())*(v1[attribute2].sum()))/total))/(((v[i].sum())*(v1[attribute2].sum()))/total)
        ...
        if chiSquare > degreeOfFreedom:
            res = "Attributes " + attribute1 + ' and ' + \
                attribute2 + " are strongly correlated."
        else:
    ...
    if operation == "Correlation(Pearson) Coefficient":
        ...
        sum = 0
        for i in range(len(data)):
            sum += (data.loc[i, attribute1]-avg1) * \
                (data.loc[i, attribute1]-avg1)
        var1 = sum/(len(data))
        sd1 = math.sqrt(var1)
        ...
        sum = 0
        for i in range(len(data)):
            sum += (data.loc[i, attribute2]-avg2) * \
                (data.loc[i, attribute2]-avg2)
        var2 = sum/(len(data))
        sd2 = math.sqrt(var2)
        ...
        if pearsonCoeff > 0:
            res = "Attributes " + attribute1 + ' and ' + \
                attribute2 + " are positively correlated."
        elif pearsonCoeff < 0:
            res = "Attributes " + attribute1 + ' and ' + \
                attribute2 + " are negatively correlated."
        elif pearsonCoeff == 0:
    ...
    if operation == "Normalization Techniques":
        ...
        function = st.selectbox("Normalization Methods",
                                normalizationOperations)
        atr1, atr2 = st.columns(2)
        attribute1 = atr1.selectbox("Select Attribute 1", cols)
        attribute2 = atr2.selectbox("Select Attribute 2", cols, index=1)
        ...
        if function == "Min-Max normalization":
            ...
            for i in range(len(data)):
                d.loc[i, attribute1] = (
                    (data.loc[i, attribute1]-min1)/(max1-min1))

            for i in range(len(data)):
                d.loc[i, attribute2] = (
                    (data.loc[i, attribute2]-min2)/(max2-min2))
            ...
            for i in range(len(data)):
                d.loc[i, attribute1] = (
                    (data.loc[i, attribute1])/(pow(10, j1)))

            for i in range(len(data)):
                d.loc[i, attribute2] = (
                    (data.loc[i, attribute2])/(pow(10, j2)))
        st.subheader("Normalized Attributes")
        st.dataframe([d[attribute1], d[attribute2]])
        sns.set_style("whitegrid")
        sns.FacetGrid(d, hue=classatr, height=4).map(
