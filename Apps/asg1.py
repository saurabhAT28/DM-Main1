...
def app(data):
    ...
    def printf(url):
        st.markdown(
            f'<p style="color:#000;font-size:24px;">{url}</p>', unsafe_allow_html=True)

    operation = st.selectbox("Operation", [
                             "Measure Central Tendency", 'Dispersion', 'Analytical Plots'], index=0)
    cols = []
    ...
    if operation == "Measure Central Tendency":
        ...
        def Midrange():
            ...
            for i in range(len(data)):
                arr.append(data.loc[i, attribute1])
            arr.sort()
            res = "Midrange of attribute ("+attribute1+") is " + \
                str((arr[n-1]+arr[0])/2)
            printf(res)
        ...
        def VSD():
            ...
            st.write('==================================================')
            res = "Standard Deviation of attribute (" + \
                attribute1+") is  "+str(math.sqrt(var))
            printf(res)
            res = np.std(arr)
            st.write(
                f"(in built fun) Standard Deviation of {attribute1} is {res}")
    ...
    if operation == "Dispersion":
        ...
        def Range():
            ...
            arr.sort()
            res = "Range of attribute ("+attribute+") is " + \
                str(arr[len(data)-1]-arr[0])
            printf(res)
        ...
        def Quartiles():
            ...
            for i in range(len(data)):
                arr.append(data.loc[i, attribute])
            arr.sort()
            res1 = "Lower quartile(Q1) of (" + attribute + \
                ")  is " + str((len(arr)+1)/4)
            res2 = "Middle quartile(Q2) of (" + attribute + \
                ") is " + str((len(arr)+1)/2)
            res3 = "Upper quartile(Q3) is (" + attribute + \
                ") is " + str(3*(len(arr)+1)/4)
            printf(res1)
            printf(res2)
            printf(res3)

            res = "Interquartile range(Q3-Q1) of given attribute (" + \
                attribute+") is"+str((3*(len(arr)+1)/4)-((len(arr)+1)/4))
            printf(res)
        ...
        def MinMax():
            ...
            printf(res)
            res = "Maximum value of attribute (" + \
                attribute+") is "+str(arr[len(data)-1])
            # printf(res)
            printf(res)
